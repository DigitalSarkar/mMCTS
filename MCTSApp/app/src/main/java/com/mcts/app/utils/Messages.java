package com.mcts.app.utils;

/**
 * Created by Raj on 11/1/2015.
 */
public class Messages {

    public static final String DATA_DOWNLOAD="કૃપા કરીને રાહ જુઓ";
    public static final String NO_INTERNET="ઇન્ટરનેટ કનેક્શન ઉપલબ્ધ નથી";
    public static final String INVALID_PIN="ખોટો પાસવર્ડ";
    public static final String NO_MATCH_DATA="કોઈપણ મેળ ખાતી માહિતી ઉપલબ્ધ નથી";
    public static final String MINIMUM_CHARACTER="શોધવા માટે ઓછામાં ઓછા 2 અક્ષર દાખલ કરો";
    public static final String NEW_FAMILY_SAVE="કૌટુંબિક માહિતી સંગ્રહ કરવામાં આવી છે";
    public static final String FAMILY_UPDATE="કૌટુંબિક માહિતી સંપાદિત કરવામાં આવી છે";
    public static final String DELETE_FAMILY="કુટુંબ રદ કરવા માગો છો?";
    public static final String DELETE_FAMILY_MEMBER="સભ્ય રદ કરવા માગો છો?";
    public static final String ADD_FALIYU="ફળિયું/સોસાયટીનું નામ દાખલ કરો";
    public static final String NOT_FOUND="માહિતી નથી";
}
